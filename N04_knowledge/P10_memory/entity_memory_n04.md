...
tldr: "Persistent entity tracker for N04: canonical entities in the knowledge domain (RAG systems, vector stores, embedding models, knowledge graph tools, retrieval frameworks). Each entity includes type, status, key facts, and cross-references."
density_score: null
related:
...
Entity memory tracks canonical entities N04 works with: systems, products, frameworks, concepts.
Updated via `cex_memory_update.py` when new entities are encountered or facts change.
Loaded selectively at session start via `cex_memory_select.py --entities knowledge`.
...
| LangChain | framework | active | Chains, agents, LCEL; heavy but popular; owned by LangChain Inc |
...
| Supabase | platform | active | CEX primary -- pgvector managed, REST API, row-level security |
| FAISS | library | active | Meta, CPU/GPU, pure ANN library (no persistence), used in retriever |

### Embedding Models

| Entity | Type | Status | Key Facts |
|--------|------|--------|-----------|
| text-embedding-3-small | model | active | OpenAI, 1536 dim, default for CEX, 8191 max tokens |
...
| LangMem | library | emerging | Google DeepMind, session compression, long-term memory for agents |
| Cognee | library | emerging | Knowledge graph + vector hybrid, entity extraction |

### Knowledge Graph Systems

| Entity | Type | Status | Key Facts |
|--------|------|--------|-----------|
| Neo4j | database | active | Industry standard property graph, Cypher query, APOC plugins |
| Amazon Neptune | service | active | Managed, RDF + property graph, SPARQL + Gremlin |
| Nebula Graph | database | active | Distributed, petabyte-scale, nGQL query language |
| Wikidata | knowledge_base | active | Largest public KG, SPARQL endpoint, 100M+ entities |
...
1. Determine type: framework | service | library | model | algorithm | database | knowledge_base
...
|----------|-------------|-------|
| [[bld_architecture_benchmark_suite]] | upstream | 0.43 |
| [[bld_architecture_api_reference]] | upstream | 0.42 |
| [[bld_architecture_discovery_questions]] | upstream | 0.42 |
| [[bld_architecture_legal_vertical]] | upstream | 0.42 |
| [[bld_architecture_roi_calculator]] | upstream | 0.41 |
| [[bld_architecture_sdk_example]] | upstream | 0.41 |
| [[bld_architecture_app_directory_entry]] | upstream | 0.41 |
| [[bld_architecture_memory_benchmark]] | upstream | 0.41 |
| [[bld_architecture_fintech_vertical]] | upstream | 0.41 |
| [[bld_architecture_quickstart_guide]] | upstream | 0.41 |
