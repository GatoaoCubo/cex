...
## Boundary
vectordb_backend IS: storage and query config for embedding vectors with indexing and similarity search.
vectordb_backend IS NOT: an embedder provider (vector generation), a retriever (query pipeline), or a chunk strategy.
