---
id: p01_kc_brain_index
kind: knowledge_card
type: kind
pillar: P10
title: "Brain Index — Deep Knowledge for brain_index"
version: 1.0.0
created: 2026-03-30
updated: 2026-03-30
author: york
...
tldr: "brain_index is the configuration spec for a semantic search index — backend (BM25/FAISS/hybrid), dimensions, hybrid alpha weight, and stale rebuild threshold — scoped to a named domain."
when_to_use: "Building, reviewing, or reasoning about brain_index artifacts"
keywords: [vector_index, FAISS, hybrid_search]
feeds_kinds: [brain_index]
density_score: null
---
...
## What It Is
A brain_index is the configuration spec for a named semantic search index — defining its backend algorithm (BM25, FAISS, or hybrid), embedding dimensions, hybrid alpha weighting, and stale rebuild threshold. It is NOT an embedding_config (P01, which configures the embedding model itself), NOT a rag_source (P01, which is a pointer to an external data source).
...
## Integration Graph
```
embedding_config, rag_source --> [brain_index] --> retriever_config, agent_card
                                       |
