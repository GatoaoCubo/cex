...
| Full boot | Production satellite | Identity + mental_model + MCPs + all domain tools |
| Provider adapter | Same agent, different platforms | Claude Code boot vs Cursor boot for same SHAKA agent |
...
- IF defining satellite launch parameters THEN spawn_config (P12)
