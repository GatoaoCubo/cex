...
## What It Is
A component_map is a structured map of components and their directed connections within a bounded scope — it answers "what connects to what" with explicit nodes and labeled edges. It is NOT a diagram (visual rendering), NOT an agent_card (scoped to a single satellite identity), and NOT a workflow (no execution sequence or step ordering).
...
|-----------|------|---------|----------|
| scope | string | required | system/satellite/layer — broader = less actionable |
...
| Satellite topology | One satellite's agent + tool connections | p08_cmap_atlas.yaml — ATLAS agents + MCP tools |
| Data flow | Tracing data transformation between stages | input → chunk → embed → index flow |

## Anti-Patterns
| Anti-Pattern | Why It Fails | Fix |
|-------------|-------------|-----|
| Mega-map | All 118 agents in one map is unreadable | Scope to 1 satellite or 1 layer at a time |
...
## Decision Tree
- IF visualizing one satellite's internals THEN satellite topology map
- IF auditing data transformation THEN data flow map
- IF designing new system architecture THEN system-level layer map first
- DEFAULT: satellite topology, 1:1 with agents, one file per satellite
