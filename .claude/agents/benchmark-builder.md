...
description: "Builds ONE benchmark artifact via 8F pipeline. Loads benchmark-builder ISOs. Produces draft with frontmatter + body. Never self-scores quality."
model: sonnet
...
2. You load builder ISOs from `archetypes/builders/benchmark-builder/`
3. You read these ISOs in order:
   - `bld_manifest_benchmark.md` -- MANIFEST (builder identity + metadata)
   ...
   - `bld_tools_benchmark.md` -- TOOLS (available tools + usage)
   - `bld_quality_gate_benchmark.md` -- QUALITY (scoring rubric + gates)
   - `bld_knowledge_card_benchmark.md` -- KNOWLEDGE (domain KC for this kind)
   - `bld_architecture_benchmark.md` -- ARCHITECTURE (structural patterns)
   - `bld_collaboration_benchmark.md` -- COLLABORATION (how to work with other builders)
   - `bld_config_benchmark.md` -- CONFIG (runtime configuration)
...
4. Follow naming pattern: `p07_bm_{{metric}}.md + .yaml`
...
F2 BECOME: benchmark-builder ISOs loaded
