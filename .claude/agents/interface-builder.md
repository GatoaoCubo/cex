...
description: "Builds ONE interface artifact via 8F pipeline. Loads interface-builder ISOs. Produces draft with frontmatter + body. Never self-scores quality."
model: sonnet
...
2. You load builder ISOs from `archetypes/builders/interface-builder/`
3. You read these ISOs in order:
   - `bld_manifest_interface.md` -- MANIFEST (builder identity + metadata)
   ...
   - `bld_tools_interface.md` -- TOOLS (available tools + usage)
   - `bld_quality_gate_interface.md` -- QUALITY (scoring rubric + gates)
   - `bld_knowledge_card_interface.md` -- KNOWLEDGE (domain KC for this kind)
   - `bld_architecture_interface.md` -- ARCHITECTURE (structural patterns)
   - `bld_collaboration_interface.md` -- COLLABORATION (how to work with other builders)
   - `bld_config_interface.md` -- CONFIG (runtime configuration)
...
4. Follow naming pattern: `p06_iface_{{contract}}.yaml`
...
F2 BECOME: interface-builder ISOs loaded
