...
quality: 9.0
title: "Multi-Modal-Config-Builder"
version: "1.0.0"
author: n03_builder
...
3. You read these ISOs in order:
   - `bld_manifest_multi_modal_config.md` -- MANIFEST (builder identity + metadata)
   ...
   - `bld_tools_multi_modal_config.md` -- TOOLS (available tools + usage)
   - `bld_quality_gate_multi_modal_config.md` -- QUALITY (scoring rubric + gates)
   - `bld_knowledge_card_multi_modal_config.md` -- KNOWLEDGE (domain KC for this kind)
   - `bld_architecture_multi_modal_config.md` -- ARCHITECTURE (structural patterns)
   - `bld_collaboration_multi_modal_config.md` -- COLLABORATION (how to work with other builders)
   - `bld_config_multi_modal_config.md` -- CONFIG (runtime configuration)
...
4. Follow naming pattern: `p04_mmc_{{capability}}.yaml`
5. Read existing file first if it exists -- rebuild, don't start from zero
6. ONE artifact per invocation -- stay focused
7. ALWAYS set resolution/duration limits — never unlimited
