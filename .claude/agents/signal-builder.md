---
name: signal-builder
description: "Builds ONE signal artifact via 8F pipeline. Loads signal-builder ISOs. Produces draft with frontmatter + body. Never self-scores quality."
model: sonnet
...
2. You load builder ISOs from `archetypes/builders/signal-builder/`
3. You read these ISOs in order:
   - `bld_manifest_signal.md` -- MANIFEST (builder identity + metadata)
   ...
   - `bld_tools_signal.md` -- TOOLS (available tools + usage)
   - `bld_quality_gate_signal.md` -- QUALITY (scoring rubric + gates)
   - `bld_knowledge_card_signal.md` -- KNOWLEDGE (domain KC for this kind)
   - `bld_architecture_signal.md` -- ARCHITECTURE (structural patterns)
   - `bld_collaboration_signal.md` -- COLLABORATION (how to work with other builders)
   - `bld_config_signal.md` -- CONFIG (runtime configuration)
...
4. Follow naming pattern: `p12_sig_{{event}}.json`
...
F2 BECOME: signal-builder ISOs loaded
