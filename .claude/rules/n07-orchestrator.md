...
## Pre-flight MCP Protocol (PREFLIGHT_EXPANSION)

N07 is the sole MCP gateway for external context gathering. Before dispatching
to non-Claude runtimes, N07 pre-compiles external context into the handoff.

### When to run Phase 0

```python
kind_meta = kinds_meta[resolved_kind]
if kind_meta.get("requires_external_context", False):
    # Run Phase 0 -- gather external context via MCP
    context = cex_preflight_mcp.gather(task, kind, budget_tokens=8192)
    handoff += format_external_context_section(context)
# else: skip Phase 0 -- structural kinds need no external data
```

### Phase 0 Execution

```bash
# Pre-flight runs in N07 session (sole MCP holder)
python _tools/cex_preflight_mcp.py --kind {kind} --task "{task}" \
    --budget 8192 --timeout 60 --output .cex/cache/preflight/

# Providers (tiered):
#   DEFAULT (free): github(read) + fetch + markitdown
#   OPTIONAL: firecrawl (FIRECRAWL_API_KEY), brave-search (BRAVE_API_KEY)
# Config: .cex/config/preflight_sources.yaml
```

### Kinds That Require Live Tools at Runtime

A small set (~5 kinds) need MCP during generation, not just pre-flight.
These MUST route to Claude runtime regardless of grid configuration:

| Kind | Reason |
|------|--------|
| `browser_tool` | Live browser interaction |
| `mcp_server` | Tests live MCP connections |
| `interactive_demo` | Live browser rendering |
| `computer_use` | Live screen interaction |
| `db_connector` | Live database connection |

Router enforcement: `cex_router_v2.py` checks `requires_live_tools` field
and hard-routes to Claude. Config: `.cex/kinds_meta.json`.

### Security Constraints

- GitHub MCP: ONLY `get_*`, `list_*`, `search_*` allowed. All mutations denied.
- No secrets logged. `GITHUB_TOKEN` from env only, never hardcoded.
- Graceful skip when API keys absent -- pre-flight never fails hard.
- Full policy: `_docs/specs/spec_mcp_security_policy.md`
...
|----------|-------------|-------|
| [[p01_kc_orchestration]] | related | 0.49 |
| [[p03_sp_admin_orchestrator]] | related | 0.48 |
| [[p01_kc_orchestration_best_practices]] | related | 0.46 |
| [[p08_ac_admin_orchestrator]] | related | 0.45 |
| [[dispatch]] | related | 0.43 |
| [[p01_kc_cex_orchestration_architecture]] | related | 0.43 |
| [[p12_wf_admin_orchestration]] | related | 0.42 |
| [[p03_sp_orchestration_nucleus]] | related | 0.41 |
| [[p02_agent_admin_orchestrator]] | related | 0.40 |
| [[p08_ac_orchestrator]] | related | 0.36 |
