...
N03 owns 4 composable crews for multi-role engineering workflows:

| Crew | Process | Roles | Purpose |
|------|---------|------:|---------|
| `artifact_factory` | sequential | 4 | Spec decomposition -> 8F build -> peer review -> consistency integration |
| `code_review_pipeline` | sequential | 3 | Regression scan -> security audit -> LLM-as-judge quality assessment |
| `builder_factory` | sequential | 3 | Kind analysis + spec -> 12-ISO production -> doctor + compile validation |
| `migration_pipeline` | sequential | 3 | State mapping + migration plan -> in-place transformation -> regression validation |

### Role Inventory (13 roles across 4 crews)

| Role | Crew | Agent Binding | Function |
|------|------|--------------|----------|
| architect | artifact_factory | workflow-builder | Decompose spec into typed artifact list |
| builder | artifact_factory | kind-builder | Produce each artifact via full 8F pipeline |
| reviewer | artifact_factory | quality-gate-builder | Score each artifact; reject below 9.0 |
| integrator | artifact_factory | diagram-builder | Cross-reference + doctor + crew-complete |
| scanner | code_review_pipeline | regression-check-builder | Detect regressions, breaking changes |
| security_auditor | code_review_pipeline | threat-model-builder | Assess vulnerabilities, credential exposure |
| quality_judge | code_review_pipeline | llm-judge-builder | LLM-as-judge final score + publish decision |
| spec_writer | builder_factory | constraint-spec-builder | Analyze kind schema; produce builder specification |
| iso_generator | builder_factory | kind-builder | Produce all 12 ISOs from spec |
| test_runner | builder_factory | smoke-eval-builder | Validate ISOs via doctor + compile + naming |
| analyzer | migration_pipeline | component-map-builder | Map current state; produce migration plan |
| transformer | migration_pipeline | kind-builder | Apply transformations in dependency order |
| migration_validator | migration_pipeline | regression-check-builder | Post-migration regression check |

### Crew Composition Patterns

- **artifact_factory + code_review_pipeline**: Chain for reviewed builds. Factory produces, review pipeline validates.
- **builder_factory + migration_pipeline**: Chain for taxonomy evolution. Factory scaffolds new builders, migration pipeline updates existing ones.
- **Grid of crews**: N07 can dispatch multiple crew instances in parallel (e.g., 3 builder_factory runs for 3 new kinds simultaneously).

All crew files: `N03_engineering/P12_orchestration/crews/`

## Related Artifacts

| Artifact | Relationship | Score |
|----------|-------------|-------|
| [[kc_intent_resolution_map]] | related | 0.38 |
| [[p02_agent_builder_nucleus]] | downstream | 0.35 |
| [[p12_dr_builder_nucleus]] | downstream | 0.34 |
| [[p03_sp_n03_creation_nucleus]] | downstream | 0.34 |
| [[p02_nd_n03.md]] | downstream | 0.34 |
| [[p01_kc_cex_project_overview]] | related | 0.33 |
| [[p12_dr_software_project]] | downstream | 0.33 |
| [[p02_agent_creation_nucleus]] | downstream | 0.32 |
| [[p03_pc_cex_universal]] | downstream | 0.32 |
| [[agent_card_engineering_nucleus]] | downstream | 0.32 |
