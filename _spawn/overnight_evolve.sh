...
# --- PHASE 0: Cluster evolution (Karpathy multi-file mutation) ---
phase_cluster() {
    log "PHASE 0: Cluster evolution"
    # Pick a random artifact below 9.0 that has related: populated
    TARGET=$(python -c "
import os, re, random
candidates = []
for root, dirs, files in os.walk('.'):
    dirs[:] = [d for d in dirs if d not in {'.git','.obsidian','__pycache__','node_modules','.cex'}]
    for f in files:
        if not f.endswith('.md'):
            continue
        path = os.path.join(root, f)
        try:
            content = open(path, encoding='utf-8', errors='ignore').read(800)
        except OSError:
            continue
...
" 2>/dev/null)
    ...
    if [ -n "$TARGET" ]; then
        log "  Cluster target: $TARGET"
        python _tools/cex_evolve.py cluster "$TARGET" --max-neighbors 5 --max-rounds 3 2>&1 | tail -10
    else
        log "  No cluster candidates found"
    fi
}
...
# Post-run analytics report
log "Generating experiment analytics..."
python _tools/cex_experiment_analytics.py 2>&1 | tee -a "$LOG_FILE"
if [ -f ".cex/experiments/diff_log.jsonl" ]; then
    log "Cluster evolution stats:"
    python _tools/cex_experiment_analytics.py --diff-log 2>&1 | tee -a "$LOG_FILE"
fi
