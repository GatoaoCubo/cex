...
claude -p --dangerously-skip-permissions --permission-mode bypassPermissions --no-chrome --model claude-opus-4-6 --name "OVERNIGHT-G1.3" --output-format text "Read .cex/kinds_meta.json. The naming patterns are WRONG for most kinds. Reality: KCs use 'kc_{{topic}}.md', templates use 'tpl_{{kind}}.md', builders use 'bld_{{role}}_{{kind}}.md'. Update the 12 kinds that have 'NONE' as naming to have a real pattern based on their pillar prefix. Also fix the 2 pillar mismatches: axiom should be P02 or naming should match P02, and naming_rule should be P08 or naming should match P08. Then git add and commit with '[N04] fix: kinds_meta naming patterns match reality (roadmap principle 2)'." >> %LOG% 2>&1
