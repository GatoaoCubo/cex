...
STOP_WORDS = {
    ...
    "the", "a", "an", "and", "or", "of", "to", "in", "for", "with", "on", "is", "it",
...
def load_builders(conn: sqlite3.Connection) -> list[dict]:
    """Load all bld_manifest rows from the index."""
    rows = conn.execute(
        "SELECT id, kind, domain, keywords_json, pillar, path FROM files "
        "WHERE path LIKE '%bld_manifest%'"
    ).fetchall()
    builders = []
    for row in rows:
        kw_raw = row[3] or "[]"
...
def suggest_crew(query_text: str, db_path: Path = DB_PATH) -> list[dict]:
    """Return suggested crew from bld_collaboration for top-matching builder."""
    ...
    # Try to find collaboration file
    collab_pattern = f"archetypes/builders/{builder_dir}"
    conn = sqlite3.connect(str(db_path))
    rows = conn.execute(
        "SELECT path FROM files WHERE path LIKE ? AND path LIKE '%collaboration%'",
        (f"%{builder_dir}%",)
    ).fetchall()
    conn.close()

    if not rows:
        return [{"builder": top["builder_id"], "crew": "no collaboration file found"}]

    # Read the collaboration file
    collab_path = ROOT / rows[0][0]
    if collab_path.exists():
        try:
            text = collab_path.read_text(encoding="utf-8", errors="replace")
            return [{"builder": top["builder_id"], "collaboration": text[:500]}]
        except Exception:
            pass

    return [{"builder": top["builder_id"], "crew": "collaboration file unreadable"}]
    ...
    if args.suggest_crew:
        ...
        if args.json:
            ...
            for c in crew:
                ...
                if "collaboration" in c:
                    print(c["collaboration"])
                else:
