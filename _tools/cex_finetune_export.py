...
For each of the 123 kinds, creates a training pair:
  ...
  Output: bld_instruction_{kind}.md content (the ideal artifact spec)
...
def build_output(builder_dir: Path, kind_name: str) -> str | None:
    ...
    if not instruction_file.exists():
        ...
        instruction_file = builder_dir / f"bld_instruction_{hyphen_name}.md"
    if not instruction_file.exists():
        # Search for any bld_instruction_*.md
        candidates = list(builder_dir.glob("bld_instruction_*.md"))
        if candidates:
...
def main() -> None:
    ...
    # Stats
    print(f"\n{'='*50}")
    print(f"FINE-TUNE EXPORT SUMMARY")
    print(f"{'='*50}")
