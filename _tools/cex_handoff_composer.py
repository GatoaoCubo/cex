...
def compose_handoff(
    ...
    if kind_contexts:
        ...
        for ctx in kind_contexts:
            ...
            lines.append(f"| Resource | Path | Status |")
            lines.append(f"|----------|------|--------|")
            ...
            if ctx.builder_dir:
                ...
                for iso in ctx.iso_paths:
                    stem = iso.stem.replace(f"bld_", "").replace(f"_{ctx.kind}", "")
                    lines.append(f"| ISO: {stem} | `{_rel(iso)}` | OK |")
            else:
                lines.append(f"| Builder dir | (not found) | MISSING |")

            # KC
            if ctx.kc_path:
                lines.append(f"| Knowledge Card | `{_rel(ctx.kc_path)}` | OK |")
            else:
                lines.append(f"| Knowledge Card | (not found) | MISSING |")
...
""",
    ...
    parser.add_argument("--discover-only", action="store_true",
                        help="Only discover kinds, don't compose handoff")
