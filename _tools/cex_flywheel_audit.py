...
def audit_L3() -> None:
    ...
    for bd in bdirs[:5]:
        ...
        record("L3", f"{bd.name}/ISOs",
               "WIRED" if len(isos) >= 12 else "BROKEN", f"{len(isos)}/12")
...
def audit_wires() -> None:
    ...
    try:
        ...
        from cex_crew_runner import (RunState, check_compaction_needed,
                                     check_memory_extract_needed,
                                     compose_prompt)
        from cex_prompt_layers import get_layers
        layers = get_layers()
    except Exception as e:
...
def audit_cascades() -> None:
    ...
    bk = CEX_ROOT / "archetypes" / "builders" / "agent-builder" / "bld_knowledge_agent.md"
...
def print_report() -> None:
    ...
    t = len(RESULTS)
    print(f"\n{'='*60}")
    print("CEX FLYWHEEL AUDIT -- Doc vs Practice")
    print(f"{'='*60}")
