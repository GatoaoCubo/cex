...
def build_claude_ps1(nucleus: str, cfg: dict, meta: dict) -> str:
    ...
    mcp_arg = f'--mcp-config "{ROOT}\\{mcps}"' if mcps else ""
    settings_arg = f'--settings "{ROOT}\\{settings}"' if settings else ""
    ...
    if sin_injection:
        ...
        safe_injection = sin_injection.replace('"', '`"').replace('\n', ' ')
...
claude {flags} --model {model} {mcp_arg} {settings_arg} "{full_prompt}"
'''
