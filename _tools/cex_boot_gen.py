...
# Primary (and only) builders -- PowerShell-only stack
BUILDERS = {
...
def generate(config: dict, only: str = None, dry_run: bool = False):
    ...
    for nucleus, cfg in config.items():
        ...
        if not builder:
            print(f"[ERROR] Unknown CLI '{cli}' for {nucleus} -- only 'claude' supported (PowerShell-only stack)")
            continue

        # Always emit .ps1 -- PowerShell-only stack
