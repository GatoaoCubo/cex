...
def build_distill_prompt(template: str, raw_content: str, source_name: str) -> str:
    ...
    return """## TASK: Distill Knowledge

You received a RAW input and a REQUIRED TEMPLATE.
Your job: FILL the template with knowledge EXTRACTED + EXPANDED from the input.

### RULES:
1. ALL {{{{PLACEHOLDER}}}} fields must be filled -- none can be left empty
2. Do NOT copy verbatim from the input -- TRANSFORM into actionable knowledge
3. EXPAND beyond the input -- use your knowledge to enrich
4. The input is a SEED, not a limit -- if the input is basic, the output must be advanced
5. Fill 'when_to_use' thinking: "an LLM with ZERO context, when would it load this?"
6. Fill 'axioms' with IMPERATIVE rules that change behavior
7. Fill 'Anti-Patterns' with errors an LLM would make without this knowledge
8. Fill 'Application' with CONCRETE use in AI agent systems context
9. density_score >= 0.85 -- each sentence must carry information, zero filler

### TEMPLATE (fill ALL fields):

{template}

### RAW INPUT (source: {source_name}):

{raw_content}

### OUTPUT:
Return ONLY the filled template. Nothing before, nothing after.
"""
