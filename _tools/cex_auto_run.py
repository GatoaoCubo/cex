...
def get_best_model(nucleus: str, providers: dict = None) -> str:
    ...
    try:
        ...
        return get_best_provider(nucleus, providers) or "claude-sonnet-4-6"
    except Exception:
...
def get_sin_context(nucleus: str) -> dict:
    ...
    try:
        from cex_theme import get_sin, get_prompt_injection, get_lens
        sin = get_sin(nucleus)
        return {
            "sin": sin.get("sin", "?"),
...
def build_single(intent: str, verbose: bool = True) -> dict:
    ...
    if verbose:
        print(f"  [3] Delegating to cex_run pipeline...")
...
def main():
    ...
    if args.mission:
        ...
        if plan_path.exists():
            ...
            print(f"  [TODO] Multi-nucleus mission execution via cex_mission_runner.py")
