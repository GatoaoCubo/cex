...
from _tools.cex_8f_motor import (classify_objects, fan_out, generate_output,
                                 load_builder_map, load_kc_library,
                                 parse_intent, rebuild_kc_index)
from _tools.cex_compile import compile_file
from _tools.cex_crew_runner import RunState, compose_prompt
...
def post_process(artifact_path: Path, builder_id: str, intent: str,
    ...
    try:
        ...
        try:
            ...
            results["scored"] = True
            if verbose:
                print("  Score:    [OK] applied (evolve fallback)")
        except Exception:
    ...
    try:
        ...
        if verbose:
            print("  Memory:   [OK] observation recorded")
    except Exception:
...
def run(intent: str, quality: float = 9.0, dry_run: bool = False,
    ...
    if not execute:
        ...
        if verbose:
            ...
            print("  Saved to: .cex/runtime/last_prompt.md")
            print("  Add --execute to send to claude CLI")
            print(f"{'='*60}")
        return result
    ...
    try:
        from _tools.cex_provider_discovery import (discover_providers,
                                                   get_best_provider)
