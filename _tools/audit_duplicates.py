...
def py_function_hashes(file_path: Path) -> list[tuple[str, str, str]]:
    ...
    for node in tree.body:
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            if len(body_norm) < 50:
                continue
            h = hashlib.sha1(body_norm.encode("utf-8"), usedforsecurity=False).hexdigest()[:16]
...
def md_minhash_signature(shingle_set: set[str], num_hashes: int = 64) -> tuple[int, ...]:
    ...
    for i in range(num_hashes):
        salt = str(i).encode()
        min_h = min(
            int.from_bytes(hashlib.sha1(salt + s.encode("utf-8"), usedforsecurity=False).digest()[:8], "big")
