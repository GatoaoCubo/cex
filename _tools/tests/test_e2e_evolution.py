...
def load_manifest(builder_name):
    ...
    p = BUILDERS_DIR / builder_name / f"bld_manifest_{kind_slug}.md"
...
class TestBuilderEffortModel:
    ...
    def test_effort_profile_builder_exists(self):
        """effort-profile-builder is a functional builder with 13 ISOs."""
    ...
    def test_effort_levels_documented(self):
        ...
        kc = BUILDERS_DIR / "effort-profile-builder" / "bld_knowledge_card_effort_profile.md"
...
class TestHookLifecycle:
    ...
    def test_hook_config_builder_exists(self):
        """hook-config-builder has 13 ISOs."""
    ...
    def test_hook_config_documents_phases(self):
        ...
        kc = BUILDERS_DIR / "hook-config-builder" / "bld_knowledge_card_hook_config.md"
        content = kc.read_text(encoding="utf-8")
...
class TestQualityMemory:
    ...
    def test_quality_gates_exist(self):
        ...
        for builder in NEW_BUILDERS:
            kind_slug = builder.replace("-builder", "").replace("-", "_")
            qg = BUILDERS_DIR / builder / f"bld_quality_gate_{kind_slug}.md"
            assert qg.exists(), f"{builder} missing quality gate"
...
class TestSchemaIntegrity:
    ...
    def test_manifests_have_keywords(self):
        ...
        for bd in BUILDERS_DIR.iterdir():
            ...
            manifest = bd / f"bld_manifest_{kind_slug}.md"
            if manifest.exists():
                manifest_count += 1
