...
from brand_audit import (score_archetype_alignment, score_config_completeness,
                         score_narrative_integrity, score_positioning_clarity,
                         score_visual_coherence, score_voice_consistency)
from brand_inject import (INSTANCE_CONFIG, INSTANCE_VARS, compute_derived,
                          count_unresolved, flatten, inject_brand,
                          load_brand_config, load_instance_config)
from brand_propagate import NUCLEUS_VARS, extract_vars, format_brand_context
from brand_validate import is_placeholder, validate
...
class TestScorePositioningClarity:
    def test_full_positioning(self):
        config = {"positioning": {"BRAND_CATEGORY": "testing", "BRAND_UVP": "The only testing framework that writes itsel", "BRAND_DIFFERENTIATOR": "self-generating tests"}}
