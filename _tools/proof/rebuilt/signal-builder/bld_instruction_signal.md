...
3. Append optional fields (only if each meets ALL criteria):
   ...
   Common valid optional fields:
     ...
     "artifacts_count": integer count
     "commit_hash":   git commit reference
     "error_code":    short error classifier
     "message":       brief human-readable note
     "progress_pct":  0-100 (only when status=progress)
4. Size check:
...
1. HARD quality gates (all must pass):
   HARD_1: satellite is non-empty string
   HARD_2: status is one of complete/error/progress
   HARD_3: quality_score is number 0.0-10.0
   HARD_4: timestamp is valid ISO 8601 datetime string
   HARD_5: total JSON size (minified) <= 4096 bytes
   HARD_6: JSON parses without syntax errors
   HARD_7: no routing instructions or task lists in payload
2. SOFT quality checks:
   - Optional fields are compact and machine-readable
   - progress_pct only appears with status=progress
   - message is under 100 characters
3. If any HARD gate fails: fix and re-validate
4. If all pass: write to p12_sig_{{event_slug}}.json
OUTPUT: validated signal JSON file
```
