...
```
CHECK: all HARD gate IDs are sequential H01, H02, ... (no gaps)
CHECK: all SOFT gate IDs are sequential S01, S02, ... (no gaps)
CHECK: SOFT gate weights sum to exactly 1.0 (within float tolerance 0.001)
CHECK: scoring formula is explicitly stated (not implicit)
CHECK: bypass policy specifies approver role (not vague "someone")
CHECK: no gate uses subjective language without a numeric anchor
CHECK: id matches pattern p11_qg_{slug} and equals intended filename stem
CHECK: quality field is null
FAIL any check -> revise before delivery
```
Deliverable: gate spec that passes all meta-checks, ready for commit.
