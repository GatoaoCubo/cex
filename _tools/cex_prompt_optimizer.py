...
from cex_shared import find_builder_dir, load_iso, parse_frontmatter
...
def scan_builders() -> list[dict]:
    ...
    iso_prefixes = [
        "bld_manifest", "bld_system_prompt", "bld_instruction",
        "bld_knowledge_card", "bld_examples", "bld_schema",
        "bld_output_template", "bld_quality_gate", "bld_tools",
        "bld_config", "bld_memory", "bld_architecture", "bld_collaboration",
    ]
...
def _score_iso_content(body: str, prefix: str) -> float:
    ...
    if prefix == "bld_instruction" and lines > 10:
        score += 0.5
    if prefix == "bld_examples" and body.count("---") >= 2:
        score += 0.5
...
def suggest_improvements(kind: str) -> list[str]:
    ...
    if not instr:
        ...
        if len(body) < 500:
            suggestions.append(
                "bld_instruction is thin (<500 chars). Add: step-by-step process, "
                "field decision rules, density targets, section requirements"
            )
        if "density" not in body.lower():
            suggestions.append("bld_instruction doesn't mention density target (should be >= 0.85)")
        if "frontmatter" not in body.lower():
            suggestions.append("bld_instruction doesn't mention frontmatter requirements")
