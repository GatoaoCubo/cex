...
RESEARCH_PROMPT = """## TASK: Research & Taxonomy Extraction

You are a technical researcher. Your task: create a STRUCTURED TAXONOMY
for the topic "{topic}" using the sources listed below.

### OFFICIAL SOURCES (use ONLY official documentation):
{sources_list}

### CATEGORIES for classifying each term:
{categories_list}

### RULES:
1. EACH table row must have ALL 5 columns filled
2. Term = official name (class, function, concept) as it appears in the docs
3. Source = which framework/provider (e.g. langchain, openai, anthropic)
4. Category = one of the categories above
5. Description = 1 descriptive sentence (max 120 chars), NO unnecessary jargon
6. CEX Mapping = which CEX pillar/kind this knowledge feeds
   Pillars: P01(Knowledge) P02(Model) P03(Prompt) P04(Tools) P05(Output)
            P06(Schema) P07(Evals) P08(Architecture) P09(Config)
            P10(Memory) P11(Feedback) P12(Orchestration)
7. Minimum 15 terms per source. If the source has more, include more.
8. Sort by Source, then Category, then Term
9. Do NOT invent terms -- only terms that exist in official documentation
10. Include version/date when the doc was consulted
...
Return ONLY the markdown. Nothing before, nothing after.
"""
...
def build_source_scaffold(topic: str, sources: list[str], categories: list[str]) -> str:
    ...
    header = """# {topic.title()} Official Taxonomy
