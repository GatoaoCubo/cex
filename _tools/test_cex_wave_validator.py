...
def test_c1_system_prompt_llm_function(tmp: Path) -> None:
    ...
    # PASS: correct BECOME
    good = make_iso(tmp, "bld_system_prompt_voice_pipeline.md",
...
def test_c3_h02_id_pattern(tmp: Path) -> None:
    ...
    # PASS: id with {slug}-builder suffix (manifest convention)
    good = make_iso(tmp, "bld_manifest_voice_pipeline.md",
                    clean_fm(id="voice-pipeline-builder",
                             kind="manifest", pillar="P00",
                             llm_function="BECOME"),
                    body="Voice pipeline builder manifest -- STT and TTS.")
    errs = wv.validate_file(good)
...
def test_c5_no_wrong_domain_keywords(tmp: Path) -> None:
    ...
    good = make_iso(tmp, "bld_instruction_voice_pipeline.md",
                    clean_fm(id="bld_instruction_voice_pipeline",
                             kind="instruction", pillar="P03",
                             llm_function="REASON"),
...
def test_c6_placeholders_resolved(tmp: Path) -> None:
    ...
    # PASS: output_template is exempt (placeholders are the point)
    exempt = make_iso(tmp, "bld_output_template_voice_pipeline.md",
                      clean_fm(id="bld_output_template_voice_pipeline",
                               kind="output_template", pillar="P05",
...
def test_integration_bad_dir(tmp: Path) -> None:
    ...
    # C1 defect
    make_iso(bad_dir, "bld_system_prompt_voice_pipeline.md",
             clean_fm(llm_function="INJECT"),
             body="STT and TTS pipeline.")
...
TESTS = [
    test_c1_system_prompt_llm_function,
