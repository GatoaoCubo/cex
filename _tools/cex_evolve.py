...
def git_commit_multi(filepaths: list[Path], msg: str):
    ...
    for fp in filepaths:
        subprocess.run(["git", "add", str(fp)], capture_output=True)
    subprocess.run(["git", "commit", "-m", msg], capture_output=True)
...
Nothing else."""
        ...
        if new_quality > best_quality:
            ...
            log_diff(str(fp), round_num, hypothesis, "no improvement",
...
def evolve_cluster(filepath: Path, target: float = 9.0, max_rounds: int = 5,
                   max_neighbors: int = 5, verbose: bool = True) -> dict:
    ...
    primary_result = evolve_single(fp, target=target, max_rounds=max_rounds, verbose=verbose)
    ...
    # Keep or discard entire cluster
    kept = aggregate_delta >= 0
    if not kept:
        if verbose:
            print(f"\n  [!] Cluster quality dropped ({aggregate_delta:+.1f}). Reverting ALL.")
        git_restore_multi(cluster)
    else:
        if verbose:
            print("\n  [OK] Cluster quality improved or stable. Keeping ALL.")
        git_commit_multi(cluster,
                         f"[evolve-cluster] {fp.name}: {cluster_size} files, "
                         f"delta={aggregate_delta:+.1f}")
...
def main():
    ...
    if mode == "agent":
        ...
        if not parsed.file:
            print("Usage: cex_evolve.py cluster <file> [--target N] [--max-rounds N] [--max-neighbors N]")
            return
        filepath = Path(parsed.file)
        args = _parse_cli_args([sys.argv[0], mode, parsed.file] + parsed.rest)
