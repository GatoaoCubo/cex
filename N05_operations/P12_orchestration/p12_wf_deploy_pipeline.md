...
title: "CEX Artifact Deploy Pipeline"
steps_count: 10
execution: sequential
agent_groups: [n05_operations]
timeout: 1800
...
Authoritative CEX artifact deploy pipeline. Gating Wrath: every stage is a hard gate -- failure blocks promotion. Steps 1-6 retry max 2 then abort. Step 7 loops back to BUILD on quality < 8.0 (no cap). Steps 8-9 manual investigation on failure. Step 10 human-gated -- never auto-pushed.
...
| 4 | TEST | n05_operations | `cex_system_test.py` | repo + artifact | 54/54 pass | `test_ok` | fix tests, retry max 2; escalate N03 |
| 5 | DOCTOR | n05_operations | `cex_doctor.py` | repo state | 118 PASS | `doctor_ok` | remediate artifact, retry max 2 |
...
| 8 | STAGE | n05_operations | `git add` + `git commit` | artifact + score | SHA with `[N0X]` prefix | `stage_ok` | investigate conflict; manual only |
| 9 | SIGNAL | n05_operations | `signal_writer.write_signal` | nucleus, score, SHA | signal in `.cex/runtime/signals/` | `signal_ok` | check permissions; write manually |
| 10 | DEPLOY | operator (human) | `git push origin main` | commit + signal | remote updated | `deploy_ok` | never auto-push; operator decides |

**Dependencies**: each step requires prior step signal. Step 7 failure returns to Step 2. Step 10 requires Steps 8 and 9.

## Dependencies

1. `cex_hooks.py` registered as pre-commit hook (`_tools/`)
...
1. `_tools/cex_hooks.py` -- ASCII + frontmatter checks
2. `_tools/cex_compile.py` -- .md to .yaml
3. `_tools/cex_system_test.py` -- 54 tests
4. `_tools/cex_doctor.py` -- 118 health checks
5. `_tools/cex_sanitize.py` -- ASCII enforcement
6. `_tools/cex_score.py` -- 3-layer scoring (30/30/40)
...
| [[p12_wf_engineering_pipeline]] | sibling | 0.35 |
| [[tpl_instruction]] | upstream | 0.33 |
| [[p12_wf_knowledge_pipeline]] | sibling | 0.30 |
| [[p10_lr_chain_builder]] | upstream | 0.30 |
| [[p12_wf_create_orchestration_agent]] | sibling | 0.30 |
| [[p11_qg_workflow]] | related | 0.30 |
| [[p12_wf_orchestration_pipeline]] | sibling | 0.30 |
| [[p12_wf_creation_pipeline]] | sibling | 0.29 |
| [[bld_instruction_chain]] | upstream | 0.29 |
