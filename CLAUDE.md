...
Most "AI agents" are a system prompt plus a few tools. CEX treats AI as typed
...
layer that compounds over time. Intelligence compounds faster when exchanged.

Three properties:

- **Composable** — 8F × 12 pillars × 300 kinds = the factory floor. Spawn a nucleus in minutes.
- **Sovereign** — runs on Claude, GPT, Gemini, or Ollama. Knowledge lives in *your* repo.
- **Self-assimilating** — every conversation compiles into typed, governed, searchable knowledge assets. Every artifact is an exchange unit — portable across instances, runtimes, and teams.
...
|---------|------|----------|------------|
| N01 | intelligence | Analytical Envy | Sonnet |
| N02 | marketing | Creative Lust | Sonnet |
| N03 | engineering | Inventive Pride | **Opus** |
| N04 | knowledge | Knowledge Gluttony | Sonnet |
| N05 | operations | Gating Wrath | Sonnet |
| N06 | commercial | Strategic Greed | Sonnet |
| N07 | orchestrator | Orchestrating Sloth | **Opus** |
...
| **Builders (source of truth)** | `archetypes/builders/{kind}-builder/` (12 ISOs each, 1:1 with pillars) |
| **Sub-agents** | `.claude/agents/{kind}-builder.md` (301 files) |
...
| **Kind KCs** | `N00_genesis/P01_knowledge/library/kind/kc_{kind}.md` (299 files) |
...
| **Example crew** | `N02_marketing/P12_orchestration/p12_ct_product_launch.md` (4-role sequential) |
...
| **Subagent definitions** | `.claude/agents/` (301 builder sub-agents) |
| **Nucleus fractals** | `N{01-07}_*/` (13 subdirs each, mirrors N00's 12 pillars) |
| **SDK runtime** | `cex_sdk/` (100+ .py modules) |
...
| **Prompt Compiler** (intent transmutation source-of-truth, 300 kinds, multilingual EN-first) | `N00_genesis/P03_prompt/layers/p03_pc_cex_universal.md` -- referenced by `n07-input-transmutation.md` |
...
| **Token Efficiency Map** | `N01_intelligence/reports/token_efficiency_gap_map.md` (TODO: create) |
| **Taxonomy Audit** | `N01_intelligence/reports/taxonomy_completeness_audit.md` (TODO: create) |
...
| **Media pipeline** | `_tools/cex_media_produce.py` + `N05_operations/P09_config/media_config.yaml` |
| **Concept registry** | `N05_operations/P09_config/concept_registry.yaml` (10 infoproduct concepts, 3 formats, EN) |
...
| `/cex-doctor` | Full diagnostics (separate from Claude Code's native `/doctor`) |
...
| `cex_skill_loader.py` | Builder ISO loader: 12 ISOs per kind (1:1 pillars), shared defaults, conditionals |
...
| `cex_media_produce.py` | **Infoproduct pipeline**: concept x format x lens x lang -> text/audio_source/slides (10 concepts registered) |
...
| Orchestration + MCP gateway | N07 | claude | opus-4-6 | 1M | **Opus** |

> **Preflight**: `cex_preflight.py` runs 3 phases — Phase 0: MCP gather (N07 only, external context via fetch/github/markitdown); Phase 1: local TF-IDF; Phase 2: Haiku reranking. Reduces token burn ~70%. Security: `.claude/nucleus-settings/n07.json` enforces GitHub read-only.
