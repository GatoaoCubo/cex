...
CRITICAL: verify raw_body BEFORE JSON decode. Parsing before verification allows
...
```
wait = base_ms * (2 ^ attempt_number)
```
Example (base=1000ms): 1s, 2s, 4s, 8s, 16s — cap at 5 attempts for most cases.

Stripe retries up to 3 days. GitHub retries for ~72h. Design receivers to be
fast (respond 2xx in <3s) and idempotent.
...
1. Log full payload + error
2. Emit to dead-letter queue or table
3. Alert on-call if queue depth > threshold
4. Manual replay path must exist
