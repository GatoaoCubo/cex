...
4. **Gate checklist** — list each of the 8 HARD gates with PASS / PENDING status.

Maximum artifact size: 2048 bytes. Use compact rule notation if needed.

## Constraints

**In scope**: output format transformation rules, template engine selection, field mapping, escaping strategy, locale configuration, tabulation rules, formatter frontmatter authoring.

**Out of scope**: data extraction from unstructured text (parser-builder), LLM output schema definition (response-format-builder), identifier and naming conventions (naming-rule-builder), input content validation (validator-builder).
