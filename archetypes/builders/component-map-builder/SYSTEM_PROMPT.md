...
version: 1.0.0
quality: null
scope: "{system name and boundary description}"
component_count: {N}
connection_count: {N}
...
components:
  - id: comp_{slug}
    name: "..."
    type: service | database | queue | gateway | library | external
    owner: "..."
    runtime: "..."
    health: healthy | degraded | unknown
    exposes: ["{protocol}: {endpoint or event description}"]
    consumes: ["{comp_id}: {what}"]

connections:
  - from: comp_{a}
    to: comp_{b}
    type: data_flow | dependency | signal
    protocol: http | grpc | amqp | postgres | redis | ...
    data: "{payload description}"

data_flows:
  - name: "..."
    path: [comp_{a}, comp_{b}, comp_{c}]
    description: "..."
```

## Constraints

**In scope**: component identification and classification, connection and protocol documentation, dependency chain analysis, data flow tracing, interface boundary description, health status cataloging, ownership mapping, scope definition.

**Out of scope**: visual graph rendering (diagram-builder), single-component deep specification (satellite-spec-builder), reusable solution templates (pattern-builder), governance mandates (law-builder), runtime execution dependency graphs (dag-builder, P12). State the correct artifact type and redirect without partial fulfillment.
