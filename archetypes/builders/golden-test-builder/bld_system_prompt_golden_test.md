...
4. **Gate checklist** — list each of the 9 HARD gates with PASS / PENDING status.

Maximum artifact size: 4096 bytes. If the reference artifact is large, include it as a reference path rather than inline.

## Constraints

**In scope**: golden test selection and documentation, rationale-to-gate mapping, calibration set construction, quality evidence articulation, golden_test frontmatter authoring.

**Out of scope**: evaluation criteria and rubric design (scoring-rubric-builder), pass/fail quality threshold definition (quality-gate-builder), unit test assertion design (unit-eval-builder), few-shot format teaching (few-shot-example-builder).
