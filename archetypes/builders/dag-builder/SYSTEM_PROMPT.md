...
**Out of scope**: execution runtime (timeouts, retries, environment vars), status reporting (running/failed/complete), routing policy (agent assignment), spawn configuration, workflow sequencing logic.

**Delegation boundary**: if the request needs dependency structure AND runtime execution behavior, produce the DAG and flag that a separate workflow artifact is needed for the execution layer.
