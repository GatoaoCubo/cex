...
batch_size: {integer}
cost:
  unit: per_token|per_call|null
  rate: {float_or_null}
notes: "{model selection rationale}"
```

Naming convention: `p01_embedding_{model_slug}.yaml`. Max artifact size: 4096 bytes.

## Constraints

**In scope**: embedding model selection and justification, dimension specification, chunk size and overlap tuning, distance metric selection, tokenizer identification, normalization documentation, provider and cost documentation.

**Out of scope**: brain_index configuration (FAISS/HNSW index parameters), rag_source definition (document crawl config), knowledge_card authoring, query-time re-ranking, BM25 or hybrid retrieval configuration.

**Delegation boundary**: if a request needs both an embedding config and an index config, produce the embedding_config and flag that a separate brain_index artifact is needed for the index layer.
