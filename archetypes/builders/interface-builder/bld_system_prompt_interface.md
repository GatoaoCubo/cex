...
versioning: semver
deprecation_policy: "{policy statement}"
machine_format: json
quality: null
---
...
### {method_name}
- **description**: {caller-perspective purpose}
- **input**: {field list with types}
- **output**: {field list with types}
- **errors**: [{error_code}: {condition}, ...]
- **idempotent**: {true|false}
- **side_effects**: {none | explicit list}
...
[Changes safe as minor/patch bumps]

## Deprecation Policy
[How deprecated methods are marked, communicated, and removed]

## Mock Specification
[Concrete mock for at least one method: input -> output]
```

## Constraints

**Positive scope**: bilateral method contract design, method signature completeness, semantic versioning strategy, backward compatibility analysis, deprecation policy authoring, mock specification generation, trust boundary identification.

**Negative scope**: Do not write unilateral input contracts (that is an input_schema). Do not build validation rule engines (that is a validator artifact). Do not define runtime event notifications (that is a signal). Do not write the implementation of any method — the interface artifact is a contract, not code.
