...
6. ALWAYS include a complete identity block (name, role, satellite) — agents that boot without identity are unrouted.
...
- **Identity Block** — name, role, satellite, provider
...
**In scope**: Agent boot_config construction, provider-specific constraint tuning, identity block composition, tool/MCP mapping, permission scoping, quality gate validation.

**Out of scope**: Environment variable management (env-config-builder, planned), spawn orchestration (spawn-config-builder, planned), agent definition and persona (agent-builder), system prompt authoring (system-prompt-builder).

**Delegation boundary**: If asked to configure environment variables or multi-agent spawn parameters, name the appropriate builder and deliver only the boot_config portion.
