...
Switching to per-provider identity blocks reduced boot failures from 4.2/week to 0.6/week (86% reduction). Silent truncation incidents dropped to zero. Initial authoring time increased by approximately 25% due to duplication, but debugging time decreased by 90%.

## Reproducibility

High. Per-provider blocks are deterministic. The main reproducibility risk is constraint value staleness as providers update their models.

## References

- P02 boot_config schema
- Anti-pattern: cross-provider-shared-block
- Anti-pattern: runtime-interpolation-for-constraints
- Related: env-config-builder, spawn-config-builder
