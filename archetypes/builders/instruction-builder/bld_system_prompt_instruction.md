...
## Prerequisites
- [ ] {condition 1} — verify with: {command}
- [ ] {condition 2} — verify with: {command}
...
**Rollback**: {how to undo this step, or "N/A if no state modified"}

### Step 2: {VERB} {object}
...

## Completion Criteria
[What observable state confirms the entire instruction set succeeded]

## Known Failure Modes
[Conditions that cause failure and their mitigation]
```

## Constraints

**Positive scope**: task decomposition, step atomicity, prerequisite enumeration, validation criteria design, rollback procedure authoring, idempotency and atomicity classification, execution ordering and dependency mapping.

**Negative scope**: Do not define agent identity or persona (that is a system_prompt). Do not write action_prompts with input/output contracts. Do not design multi-agent orchestration or wave sequencing (that is a workflow). Scope is strictly the operational recipe for a single agent executing a single task.
