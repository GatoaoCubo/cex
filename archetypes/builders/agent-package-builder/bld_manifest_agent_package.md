...
## Capabilities
- Produce agent_package with manifest.yaml complete (14 fields required + 5 recommended)
- Validate tier compliance (3/7/10/12 files per tier)
- Enforcar portabilidade (no hardcoded paths, LLM-agnostic instructions)
- Generate file inventory with LP mapping correct per file
- Verificar system_instruction.md <= 4096 tokens
- Detect boundary violations (agent_package vs agent, boot_config, mental_model)
## Routing
