...
An embedder_provider is not a vectordb_backend (no storage config), not a model_provider (no LLM routing), not a retriever (no query pipeline), and not an agent (no identity or behavior). It is the connection spec between your application and an embedding API.
...
8. NEVER configure vector storage in an embedder_provider — that is vectordb_backend's domain.
...
**Does NOT**: Configure vector databases (vectordb-backend-builder), define LLM routing (model-provider-builder), build retrieval pipelines (retriever-builder), or create agents (agent-builder). If the request requires those artifact types, reject and name the correct builder.
