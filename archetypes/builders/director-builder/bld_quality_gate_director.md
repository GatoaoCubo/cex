...
| threshold | 8.0                                                 |
| operator  | >=                                                  |
| scope     | all director artifacts (P08)                        |
## HARD Gates
...
| H04 | kind == "director" | Type integrity |
...
| H07 | llm_function == "ORCHESTRATE" | Director orchestrates, never executes |
| H08 | builders list has >= 2 entries | Single-builder director is pointless — use direct spawn |
## SOFT Scoring
| Gate | Check | Weight |
|------|-------|--------|
| S01 | tldr <= 160 chars, non-empty, not filler | 1.0 |
| S02 | tags is list, len >= 3, includes "director" | 0.5 |
...
| Score | Action |
|-------|--------|
| >= 9.5 | GOLDEN — pool as reference director definition |
| >= 8.0 | PUBLISH — register in dispatch system, deploy director |
