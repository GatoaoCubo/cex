...
pillar: P01
llm_function: INJECT
purpose: Domain knowledge for director artifact production
...
## Executive Summary
A director is the crew orchestration artifact in CEX — a declarative coordination plan that dispatches multiple builders across waves without executing tasks itself. The director kind defines HOW builders are coordinated: dispatch order, signal gates, fallback behavior, and consensus gathering. Every director requires a builders list (>= 2), explicit dispatch_mode, signal_check, wave_topology, and fallback_per_builder.
## Spec Table
| Property | Value |
|----------|-------|
...
|-------------|-------------|-----|
| Director that executes | Violates orchestration boundary — builders get confused | Director ONLY dispatches, NEVER executes |
