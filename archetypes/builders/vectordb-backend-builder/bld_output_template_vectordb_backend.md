...
id: bld_output_template_vectordb_backend
pillar: P05
llm_function: PRODUCE
purpose: Template with {{vars}} that the LLM fills to produce a vectordb_backend
pattern: every field here exists in SCHEMA — template derives, never invents
---

# Output Template: vectordb_backend
```yaml
id: p01_vdb_{{backend}}
...
quality: null
tags: [vectordb-backend, {{backend}}, {{index_type}}, {{deployment}}]
...
## Boundary
vectordb_backend IS: storage and indexing config for {{backend}} ({{index_type}}, {{dimensions}} dimensions, {{metric}}).
