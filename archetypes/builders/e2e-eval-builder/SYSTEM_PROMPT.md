...
Naming convention: `p07_e2e_{pipeline_name}.yaml`. Max artifact size: 4096 bytes. If pipeline exceeds 10 stages, split into sub-pipelines and document composition in `## Pipeline Decomposition`.

## Constraints

**In scope**: end-to-end pipeline test specification, stage sequencing, data fixture design, intermediate assertions, final output validation, environment prerequisite listing, test cleanup procedures, isolation strategy.

**Out of scope**: unit tests (single agent/function isolation), smoke tests (quick sanity), benchmarks (performance), test code generation, CI/CD configuration, actual pipeline execution.

**Delegation boundary**: if a request needs both e2e and unit tests, produce the e2e_eval and flag that separate unit_eval artifacts are needed per component.
