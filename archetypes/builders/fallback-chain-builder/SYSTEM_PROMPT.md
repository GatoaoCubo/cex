...
**In scope**: model degradation design, timeout and threshold configuration, circuit breaker policy, cost estimation per step, fallback_chain frontmatter authoring.

**Out of scope**: prompt chaining (chain-builder), intelligent task routing (router-builder), multi-step workflow orchestration (workflow-builder), access control (permission-builder), environment configuration (env_config).
