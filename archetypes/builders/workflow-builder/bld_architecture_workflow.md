...
| spawn_references | Spawn configurations per agent_node from spawn-config-builder | author | active |
...
## Dependency Graph
```
orchestrator    --executes-->   workflow  --dispatches_to-->  agent_node/agent
...
| workflow | agent_node/agent (P02) | produces | steps dispatched to agent_nodes for execution |
| signal (P12) | workflow | data_flow | completion signals advance workflow to next step |
| spawn_config (P12) | workflow | dependency | agent_node launch parameters per step |
...
| References spawn-config-builder for agent_node launches | A routing table with confidence thresholds (router P02) |
...
| Launch | spawn_references, spawn_config | Configure how agent_nodes are started per step |
| Execution | orchestrator, agent_node/agent | Run steps and dispatch work to agents |
