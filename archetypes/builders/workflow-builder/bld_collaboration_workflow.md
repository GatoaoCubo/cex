...
I design runtime orchestration: sequential and parallel steps, wave ordering, satellite coordination, signal-based completion, and error recovery strategies. I do NOT handle prompt chaining (chain-builder), static dependency graphs without execution (dag-builder), or keyword routing (dispatch_rule-builder).

## Crew Compositions

### Crew: "Mission Planning"
```
  1. spawn-config-builder -> "produces spawn_config for each satellite step in the workflow"
...
```
  ...
  2. spawn-config-builder -> "configures how each satellite is launched per step"
...
### I Receive
- seeds: mission name, satellite list, step descriptions, dependency constraints
...
- spawn-config-builder: each satellite step in the workflow references a spawn_config for launch parameters
