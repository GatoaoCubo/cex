...
- Spawn configs must be referenced per agent_node step — inline spawn parameters are error-prone
...
## Context
Workflows operate in the P12 orchestration layer as the highest-level execution construct. They coordinate multiple agents, agent_nodes, and tools across sequential and parallel execution phases. Workflows consume spawn configs (how to launch agent_nodes), signals (how to detect completion), and dispatch rules (how to route tasks). They are the runtime execution plan for complex multi-agent missions.
## Impact
