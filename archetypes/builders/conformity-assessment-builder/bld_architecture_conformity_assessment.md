...
```
User Input (system_name, category, evidence docs)
        |
        v
F1 CONSTRAIN (schema + config + manifest)
  -- Resolve Annex III category
  -- Determine Art. 43 procedure (internal / notified body)
  -- Set naming: p11_ca_{system_slug}.md
  -- Load field constraints from schema
        |
        v
F2 BECOME (system_prompt + manifest)
  -- Activate EU AI Act conformity specialist identity
  -- Apply ALWAYS/NEVER rules
  -- Confirm scope: high-risk Annex III only
        |
        v
F3 INJECT (knowledge_card + memory + examples)
  -- Load Annex III category definitions
  -- Load Annex IV section requirements
  -- Load Art. 9 RMS requirements
  -- Load Art. 10 data governance requirements
  -- Load Art. 14 human-oversight requirements
  -- Load Art. 72 PMM requirements
  -- Load learned pitfalls from memory
  -- Reference golden example (MedTriage)
        |
        v
F4 REASON (instruction -- Phase 1 RESEARCH)
  -- Enumerate RMS evidence items
  -- Gather data-governance documentation
  -- Collect human-oversight records
  -- Compile PMM data
        |
        v
F5 CALL (tools)
  -- Check schema validation (notified_body_id required if notified body)
  -- Load similar artifacts from cex_retriever
  -- Verify all required evidence is present
        |
        v
F6 PRODUCE (output_template + instruction -- Phase 2 COMPOSE)
  -- Populate all 7 Annex IV categories
  -- Insert EU AI Act citations
  -- Flag Aug-2026 deadline items
  -- Generate RMS table, DGP table, HOM controls, PMM plan
        |
        v
F7 GOVERN (quality_gate + examples -- Phase 3 VALIDATE)
  -- H01-H08 hard gates
  -- D01-D05 soft scoring
  -- Compare to MedTriage golden example
  -- If score < 8.0: return to F6
        |
        v
F8 COLLABORATE (collaboration + tools)
  -- Save to P11/ directory
  -- Compile: python _tools/cex_compile.py
  -- Score: python _tools/cex_score.py --apply
  -- Commit: git add + git commit
  -- Signal: write_signal('n03', 'complete', score)
  -- Deliver Annex-IV package to legal team
```
