...
| target_agent_node | Which execution target receives matched tasks | dispatch-rule-builder | required |
| model | LLM model assigned to the target agent_node | dispatch-rule-builder | required |
...
| fallback_agent_node | Backup target when primary agent_node is unavailable | dispatch-rule-builder | required |
...
dispatch_rule --selects--> target_agent_node (routes task to correct executor)
...
router (P02) --independent-- dispatch_rule (P02 router does multi-step model routing, DR does agent_node routing)
...
| dispatch_rule | target_agent_node | data_flow | routing decision: which agent_node + model |
| dispatch_rule | handoff | produces | selected agent_node receives handoff instructions |
| dispatch_rule | spawn_config | produces | launch parameters for selected agent_node |
...
| Includes fallback for agent_node unavailability | A spawn_config — spawn_config configures how processes are launched |
...
| Decision | scope, target_agent_node, model, priority | Specify who receives the task and with what model |
| Resilience | confidence_threshold, fallback_agent_node | Handle low-confidence matches and unavailable targets |
