...
assertions:
  - property: "{{assertion_1_property}}"
    operator: {{assertion_1_operator}}
    expected: {{assertion_1_expected}}
    gate_ref: {{assertion_1_gate_ref}}
setup: {{setup}}
teardown: {{teardown}}
timeout_s: {{timeout_s}}
quality: null
...
- [ ] H1: target_id is non-empty and references a known artifact
- [ ] H2: target_kind is one of system_prompt, action_prompt, prompt_template, instruction
- [ ] H3: test_name is kebab-case and describes the behavior being tested
- [ ] H4: input is defined and non-empty
- [ ] H5: expected_output is defined and non-empty
- [ ] H6: assertions list has >= 1 item with property, operator, and expected
- [ ] H7: timeout_s is a positive integer <= 300
...
**Does**:
- Produce a single isolated test for one agent or prompt behavior
- Require concrete expected values (no vague assertions)
- Enforce setup/teardown symmetry for state isolation
- Validate 7 HARD gates before emitting

**Does NOT**:
- Test multiple agents in sequence (e2e_eval handles pipeline tests)
- Provide quick sanity checks without assertions (smoke_eval handles that)
- Calibrate quality scoring thresholds (golden_test handles that)
- Execute the test -- it defines the test specification only

**Chaining**: unit-eval-builder output is consumed by test runners. Build unit_eval after the target artifact is complete. Use smoke_eval for fast pre-commit checks; use e2e_eval for full pipeline coverage.
