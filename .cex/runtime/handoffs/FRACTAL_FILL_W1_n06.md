...
# N06 -- Wave 1 FOUNDATIONS (8 artifacts: schemas + config)

## Mission

You are N06_commercial (Strategic Greed sin lens). Fill the P06 (schemas) and P09 (config)
...
1. `N06_commercial/architecture/nucleus_def_n06.md` -- your identity + sin lens
2. `N06_commercial/rules/` -- your nucleus rules
...
1. `N06_commercial/schemas/sch_enum_def_n06.md` -- kind=`enum_def` -- Enumeracao reutilizavel
2. `N06_commercial/schemas/sch_type_def_n06.md` -- kind=`type_def` -- Custom type definition
3. `N06_commercial/schemas/sch_validator_n06.md` -- kind=`validator` -- Validation rule (pre-commit, quality gate)

### P09 (config) -- 5 artifacts

4. `N06_commercial/config/con_env_config_n06.md` -- kind=`env_config` -- Environment variables
5. `N06_commercial/config/con_path_config_n06.md` -- kind=`path_config` -- System paths
6. `N06_commercial/config/con_permission_n06.md` -- kind=`permission` -- Permission rule (read/write/execute)
7. `N06_commercial/config/con_rate_limit_config_n06.md` -- kind=`rate_limit_config` -- Rate limiting: RPM, TPM, budget
8. `N06_commercial/config/con_secret_config_n06.md` -- kind=`secret_config` -- Secret management

## Format (every file)

```yaml
---
id: {subdir_prefix}_{kind}_n06
kind: {kind}
pillar: {pillar}
nucleus: n06
...
and a Properties table. Apply the **Strategic Greed** lens to every design choice.
