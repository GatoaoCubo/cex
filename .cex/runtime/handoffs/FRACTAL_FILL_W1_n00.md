...
# N00 -- Wave 1 FOUNDATIONS (9 artifacts: schemas + config)

## Mission

You are N00_genesis (pre-sin archetype sin lens). Fill the P06 (schemas) and P09 (config)
...
1. `N00_genesis/architecture/nucleus_def_n00.md` -- your identity + sin lens
2. `N00_genesis/rules/` -- your nucleus rules
...
1. `N00_genesis/schemas/sch_enum_def_n00.md` -- kind=`enum_def` -- Enumeracao reutilizavel
2. `N00_genesis/schemas/sch_input_schema_n00.md` -- kind=`input_schema` -- Input contract
3. `N00_genesis/schemas/sch_type_def_n00.md` -- kind=`type_def` -- Custom type definition
4. `N00_genesis/schemas/sch_validator_n00.md` -- kind=`validator` -- Validation rule (pre-commit, quality gate)

### P09 (config) -- 5 artifacts

5. `N00_genesis/config/con_env_config_n00.md` -- kind=`env_config` -- Environment variables
6. `N00_genesis/config/con_path_config_n00.md` -- kind=`path_config` -- System paths
7. `N00_genesis/config/con_permission_n00.md` -- kind=`permission` -- Permission rule (read/write/execute)
8. `N00_genesis/config/con_rate_limit_config_n00.md` -- kind=`rate_limit_config` -- Rate limiting: RPM, TPM, budget
9. `N00_genesis/config/con_secret_config_n00.md` -- kind=`secret_config` -- Secret management

## Format (every file)

```yaml
---
id: {subdir_prefix}_{kind}_n00
kind: {kind}
pillar: {pillar}
nucleus: n00
...
and a Properties table. Apply the **pre-sin archetype** lens to every design choice.
