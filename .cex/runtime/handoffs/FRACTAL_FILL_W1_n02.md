...
# N02 -- Wave 1 FOUNDATIONS (7 artifacts: schemas + config)

## Mission

You are N02_marketing (Creative Lust sin lens). Fill the P06 (schemas) and P09 (config)
pillars of your nucleus by producing 7 artifacts following the CEX 8F
pipeline and ASCII-only rule (see .claude/rules/8f-reasoning.md and
.claude/rules/ascii-code-rule.md).

## Context (READ THESE FIRST)

1. `N02_marketing/architecture/nucleus_def_n02.md` -- your identity + sin lens
2. `N02_marketing/rules/` -- your nucleus rules
...
1. `N02_marketing/schemas/sch_enum_def_n02.md` -- kind=`enum_def` -- Enumeracao reutilizavel
2. `N02_marketing/schemas/sch_type_def_n02.md` -- kind=`type_def` -- Custom type definition
3. `N02_marketing/schemas/sch_validator_n02.md` -- kind=`validator` -- Validation rule (pre-commit, quality gate)

### P09 (config) -- 4 artifacts

4. `N02_marketing/config/con_path_config_n02.md` -- kind=`path_config` -- System paths
5. `N02_marketing/config/con_permission_n02.md` -- kind=`permission` -- Permission rule (read/write/execute)
6. `N02_marketing/config/con_rate_limit_config_n02.md` -- kind=`rate_limit_config` -- Rate limiting: RPM, TPM, budget
7. `N02_marketing/config/con_secret_config_n02.md` -- kind=`secret_config` -- Secret management

## Format (every file)

```yaml
---
id: {subdir_prefix}_{kind}_n02
kind: {kind}
pillar: {pillar}
nucleus: n02
...
and a Properties table. Apply the **Creative Lust** lens to every design choice.
