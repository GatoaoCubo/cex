...
# N01 -- Wave 1 FOUNDATIONS (9 artifacts: schemas + config)

## Mission

You are N01_intelligence (Analytical Envy sin lens). Fill the P06 (schemas) and P09 (config)
pillars of your nucleus by producing 9 artifacts following the CEX 8F
pipeline and ASCII-only rule (see .claude/rules/8f-reasoning.md and
.claude/rules/ascii-code-rule.md).

## Context (READ THESE FIRST)

1. `N01_intelligence/architecture/nucleus_def_n01.md` -- your identity + sin lens
2. `N01_intelligence/rules/` -- your nucleus rules
...
1. `N01_intelligence/schemas/sch_enum_def_n01.md` -- kind=`enum_def` -- Enumeracao reutilizavel
2. `N01_intelligence/schemas/sch_input_schema_n01.md` -- kind=`input_schema` -- Input contract
3. `N01_intelligence/schemas/sch_type_def_n01.md` -- kind=`type_def` -- Custom type definition
4. `N01_intelligence/schemas/sch_validator_n01.md` -- kind=`validator` -- Validation rule (pre-commit, quality gate)

### P09 (config) -- 5 artifacts

5. `N01_intelligence/config/con_env_config_n01.md` -- kind=`env_config` -- Environment variables
6. `N01_intelligence/config/con_path_config_n01.md` -- kind=`path_config` -- System paths
7. `N01_intelligence/config/con_permission_n01.md` -- kind=`permission` -- Permission rule (read/write/execute)
8. `N01_intelligence/config/con_rate_limit_config_n01.md` -- kind=`rate_limit_config` -- Rate limiting: RPM, TPM, budget
9. `N01_intelligence/config/con_secret_config_n01.md` -- kind=`secret_config` -- Secret management

## Format (every file)

```yaml
---
id: {subdir_prefix}_{kind}_n01
kind: {kind}
pillar: {pillar}
nucleus: n01
...
and a Properties table. Apply the **Analytical Envy** lens to every design choice.
