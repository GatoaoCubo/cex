...
# N04 -- Wave 1 FOUNDATIONS (9 artifacts: schemas + config)

## Mission

You are N04_knowledge (Knowledge Gluttony sin lens). Fill the P06 (schemas) and P09 (config)
...
1. `N04_knowledge/architecture/nucleus_def_n04.md` -- your identity + sin lens
2. `N04_knowledge/rules/` -- your nucleus rules
...
1. `N04_knowledge/schemas/sch_enum_def_n04.md` -- kind=`enum_def` -- Enumeracao reutilizavel
2. `N04_knowledge/schemas/sch_input_schema_n04.md` -- kind=`input_schema` -- Input contract
3. `N04_knowledge/schemas/sch_type_def_n04.md` -- kind=`type_def` -- Custom type definition
4. `N04_knowledge/schemas/sch_validator_n04.md` -- kind=`validator` -- Validation rule (pre-commit, quality gate)

### P09 (config) -- 5 artifacts

5. `N04_knowledge/config/con_env_config_n04.md` -- kind=`env_config` -- Environment variables
6. `N04_knowledge/config/con_path_config_n04.md` -- kind=`path_config` -- System paths
7. `N04_knowledge/config/con_permission_n04.md` -- kind=`permission` -- Permission rule (read/write/execute)
8. `N04_knowledge/config/con_rate_limit_config_n04.md` -- kind=`rate_limit_config` -- Rate limiting: RPM, TPM, budget
9. `N04_knowledge/config/con_secret_config_n04.md` -- kind=`secret_config` -- Secret management

## Format (every file)

```yaml
---
id: {subdir_prefix}_{kind}_n04
kind: {kind}
pillar: {pillar}
nucleus: n04
...
and a Properties table. Apply the **Knowledge Gluttony** lens to every design choice.
