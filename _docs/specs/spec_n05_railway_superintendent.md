...
N05 deixa de ser nucleo DevOps generico e se torna **Superintendente Railway/Backend**
dono absoluto do ciclo de vida de APIs em producao.

> Pergunta-guia: Este deploy eh seguro, observavel e reversivel?
...
| KCs | 1 generico | 12 railway-native |
| Output | logs, patches | railway.toml, migrations, health endpoints |

---

## 2. HIDRATACAO -- SISTEMA REAL EM PRODUCAO

Dados extraidos do backend real (codexa-core/api/).

### 2.1 Topologia Railway (4 servicos)

| Servico | Config | Stack |
|---------|--------|-------|
...
DB: PostgreSQL Railway (asyncpg pool 3-20, SSL nego)
...
ERP: Bling OAuth2 (bg token refresh)
...
### 2.6 Startup (14 checks)

asyncpg pool > auto-migrations > Redis > rate limiter > E2B > Stripe > MercadoPago >
MercadoLivre OAuth > Bling OAuth+bg refresh > ExecutionPipeline > LLM tracking >
Credit tables > Mentor tables > Referral tables

### 2.7 Env Vars (63 total)

Obrigatorias: DATABASE_URL, ANTHROPIC_API_KEY, OPENAI_API_KEY, PORT, ENV
Mock-mode: STRIPE_SECRET_KEY, MERCADOPAGO_ACCESS_TOKEN, E2B_API_KEY
Pool: DB_POOL_MIN=3, DB_POOL_MAX=20, DB_COMMAND_TIMEOUT=60

### 2.8 Credits (BRL)

pesquisa=75c, anuncio=50c, foto=100c, full=200c, scrape=FREE
...
12 capabilities hidratadas com sistema real (ver secao 2)

### F3 INJECT (12 KCs)

| # | KC | Acao | Hidratacao |
|---|-----|------|------------|
| 1 | kc_railway_platform_deep | CREATE | 4 servicos reais |
| 2 | kc_railway_cli_patterns | CREATE | up/link/rollback |
| 3 | kc_postgresql_railway | CREATE | asyncpg SSL migrations |
| 4 | kc_nixpacks_buildpacks | CREATE | nixpacks.toml real |
| 5 | kc_zero_downtime_deploy | CREATE | 14-step startup |
| 6 | kc_api_health_monitoring | CREATE | HealthResponse real |
...
| 11 | kc_fastapi_patterns | REUSE | existente P01 |
| 12 | kc_deploy_paas | REUSE | existente P01 |
...
## 4. ARTEFATOS (30: 16 CREATE + 14 REWRITE)

W1 Identidade+Boot: agent, prompt, card, boot, mcp, seed (6)
W2 Conhecimento: 10 KCs novos + 1 rewrite (11)
W3 Schemas+Output: 6 schemas + 4 outputs (10)
W4 Quality+Orch: gate, dispatch, workflow, checkpoint (4)
