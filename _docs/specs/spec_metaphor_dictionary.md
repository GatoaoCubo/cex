...
## Lens System Metaphors (Didactic Engine)

> The /mentor command teaches CEX through 4 analogy lenses. Each lens has a complete KC:
...
> Master index (all lenses): `N04_knowledge/P01_knowledge/kc_lens_index.md`

| User says | Lens | CEX concept | Full mapping |
|-----------|------|------------|--------------|
| "assembly line" | factory | 8F pipeline | kc_lens_factory.md |
| "QA inspection" | factory | F7 GOVERN | kc_lens_factory.md |
| "work order" | factory | handoff | kc_lens_factory.md |
| "floor manager" | factory | N07 orchestrator | kc_lens_factory.md |
| "building permit process" | city | 8F pipeline | kc_lens_city.md |
| "city planner" | city | N07 orchestrator | kc_lens_city.md |
| "zoning restriction" | city | guardrail | kc_lens_city.md |
| "metabolic cycle" | biology | 8F pipeline | kc_lens_biology.md |
| "immune checkpoint" | biology | F7 GOVERN | kc_lens_biology.md |
| "hormone signal" | biology | GDP | kc_lens_biology.md |
| "quest execution" | game | 8F pipeline | kc_lens_game.md (extends game architecture above) |
| "dungeon master" | game | N07 orchestrator | kc_lens_game.md |
| "crafting station" | game | builder | kc_lens_game.md |
| "item type" | game | kind | kc_lens_game.md |
| "lore scroll" | game | knowledge_card | kc_lens_game.md |
...
## HERMES / Honcho Assimilation Metaphors (2026-04-18)

| User says | System means | Industry term | CEX implementation | Teach? |
|-----------|-------------|---------------|-------------------|--------|
| remember me across sessions | cross-session user profile | **user model** (Honcho dialectic, plastic-labs) | user_model kind (P10) + cex_user_model.py | YES -- "User model, not chat history. Honcho pattern." |
| perfil do usuario | cross-session user profile | **user model** (Honcho dialectic) | user_model kind (P10) | YES -- "User model = Honcho peer representation" |
| learn from what I do | skill auto-creation from traces | **procedural memory** (HERMES learning loop) | cex_skill_autocreate.py + skill kind | YES -- "Procedural memory, not declarative" |
| criar skill automatico | skill autonomous generation | **autonomous skill creation** (HERMES, agentskills.io) | cex_skill_autocreate.py (5+ tool calls trigger) | YES -- "Autonomous skill, not manual script" |
| persistir em MEMORY.md | agent-curated nudge | **curation nudge** (HERMES memory loop) | curation_nudge kind (P11) + cex_nudge.py | YES -- "Nudge, proactive persistence prompt" |
| self-improving agent | skill patch-in-place on reuse | **skill self-improvement** (HERMES invariant) | cex_skill_improve.py (patch, never duplicate) | YES -- "Self-improve = patch, not duplicate" |
| troca de persona | runtime persona swap | **personality** (HERMES SOUL.md pattern) | personality kind (P02) + `/personality [name]` | YES -- "Personality, hot-swap voice layer" |
| bot multiplataforma | unified messaging transport | **messaging gateway** (HERMES gateway/) | messaging_gateway kind (P04) | YES -- "Gateway, single process many platforms" |
| backend de execucao | execution environment | **terminal backend** (HERMES 6-backend abstraction) | terminal_backend kind (P09) | YES -- "Backend = where code runs (Daytona/Modal/Docker)" |
| tentativas maximas | revision iteration cap | **revision loop policy** (OpenCode-Hermes: max 3) | revision_loop_policy kind (P11) | YES -- "Revision policy, security > quality > impl" |
| cenario de pipeline | scenario-indexed agent sequence | **pipeline template** (OpenCode-Hermes 7 pipelines) | pipeline_template kind (P12) | YES -- "Pipeline = linear scenario; crew = fixed roles" |
| hibernar quando ocioso | idle-state cost guard | **hibernation policy** (serverless) | hibernation_policy kind (P09) | YES -- "Hibernation = idle serverless wake" |
| arquivo de contexto | workspace instructions | **context file** (HERMES AGENTS.md / CLAUDE.md) | context_file kind (P03) | YES -- "Context file, workspace-scoped overlay" |
| buscar conversa antiga | FTS5 session recall | **full-text session search** (HERMES FTS5 + LLM summarize) | cex_fts5_search.py | YES -- "FTS5 recall, LLM-summarized to fit context" |

## Exchange + Rebrand Metaphors (2026-04-21)

| User says | System means | Industry term | CEX implementation | Teach? |
|-----------|-------------|---------------|-------------------|--------|
| exchange | typed artifact sharing across instances | **knowledge sharing protocol** (federated learning, model merging) | spec_exchange_protocol.md -- git-based pull model with quality gates | YES -- "Knowledge sharing protocol, not file transfer" |
| artificial sin | agent behavioral bias under ambiguity | **agent personality / behavioral bias** (alignment, RLHF reward shaping) | nucleus_def `sin_lens` field -- 7 canonical sins drive optimization heuristic | YES -- "Behavioral bias, like RLHF reward signal but cultural" |
| vertical nucleus | domain-specialized AI department (N08+) | **domain-specialized agent department** (multi-agent architecture) | `N{XX}_{domain}/` -- full 12P fractal with nucleus_def, agent_card, vocabulary | YES -- "Agent department, not just an agent -- 12 pillars of capability" |
| assimilation | importing and integrating external artifacts | **integration / onboarding** (CI/CD, federated learning) | cex_doctor validates + cex_compile integrates + retriever rebuilds | YES -- "Integration with quality gates, not blind copy" |
| neuroplastic | system adapts to imported knowledge | **adaptive / extensible** (plugin architecture, online learning) | intent_resolver + prompt_compiler auto-adapt to new kinds and vocabulary | YES -- "Adaptive architecture, like a plugin system that rewires routing" |
| knowledge asset | configured CEXAI instance with compounding value | **knowledge asset** (knowledge management, intellectual capital) | CEX + brand + typed artifacts + overnight flywheel = appreciating asset | YES -- "Knowledge asset, not digital asset (avoids crypto connotation)" |
| exchange unit | self-describing shareable artifact | **artifact** / **package** (npm, PyPI, MLflow registry) | .md with YAML frontmatter (artifact) or N{XX}/ directory (nucleus package) | YES -- "Package when it's a nucleus, artifact when it's a single file" |
