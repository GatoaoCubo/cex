<p align="center">
  <h1 align="center">CEX — Cognitive Enterprise X</h1>
  <p align="center"><strong>The composable, sovereign, self-assimilating AI brain for your company.</strong></p>
  <p align="center"><em>Seven deadly sins. Twelve pillars. One typed brain. X is your company.</em></p>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/version-v10.2.0-blue" alt="Version">
...
CEX treats enterprise AI as **typed infrastructure**. Every piece of knowledge is a kind. Every kind has a builder. Every builder follows an 8-function pipeline. Seven nuclei — each one an AI department with its own toolbox, memory, crew, and cultural DNA — collaborate through a governance layer that compounds over time.

The result is not a chatbot. It is an **enterprise brain**: modular enough to grow a new department in minutes, sovereign enough to run entirely on your infra, and cumulative enough that every artifact makes the next one smarter.

### Three properties

- **Composable** — 8 functions × 12 pillars × 257 artifact kinds = the factory floor. Spawn a new nucleus, a new kind, or a new archetype in minutes.
...
> *Build your Jarvis. Own your brain.*
...
| Output (P05) | Free text | 257 typed artifact kinds + formatters + parsers |
...
- **The 8F pipeline** (CONSTRAIN → BECOME → INJECT → REASON → CALL → PRODUCE → GOVERN → COLLABORATE) drives every artifact to a 9.0+ quality floor.
...
Each operational nucleus runs on one of the seven deadly sins. The sin is not decoration — it is the cultural DNA that decides *what the nucleus optimizes for* when given ambiguous input. Values are taken verbatim from `N0X_*/architecture/nucleus_def_n0X.md`.

| Nucleus | Role | Sin Lens | Why the sin fits | Model Tier |
|---------|------|----------|------------------|------------|
| **N01** | intelligence | **Analytical Envy** *(Inveja Analítica)* | Envies every competitor, paper, and benchmark — *to surpass them* | Sonnet |
| **N02** | marketing | **Creative Lust** *(Luxúria Criativa)* | Writes copy that seduces, hooks that can't be ignored | Sonnet |
| **N03** | engineering | **Inventive Pride** *(Soberba Inventiva)* | Builds the best of its kind; refuses to ship mediocrity | **Opus** |
| **N04** | knowledge | **Knowledge Gluttony** *(Gula do Conhecimento)* | Insatiable hunger for facts, citations, context | Sonnet |
| **N05** | operations | **Gating Wrath** *(Ira)* | Merciless at the quality gate — breaks what must break | Sonnet |
| **N06** | commercial | **Strategic Greed** *(Avareza Estratégica)* | Extracts every revenue opportunity the product allows | Sonnet |
| **N07** | orchestrator | **Orchestrating Sloth** *(Preguiça Orquestradora)* | Never builds — only dispatches. Laziness as leverage | **Opus** |

N00 Genesis is the **archetype** (pre-sin) — the template from which the other seven nuclei are born.
...
| F2 | **BECOME** | Load builder identity (13 ISO files: persona, manifest, rules) |
...
```
Layer 0 — BUILDERS       259 factories × 13 ISOs each = 3,381 artifact constructors
Layer 1 — PILLARS        12 pillars × 257 kinds = the taxonomy
...
Layer 5 — TOOLS          152 Python CLI tools (cex_*.py) = the runtime
...
cex/
  ...
  _tools/                  152 Python CLI tools (cex_*.py)
  _spawn/                  Dispatch scripts (solo, grid, swarm, monitor)
  _docs/                   Whitepaper, architecture specs
  archetypes/              Builder templates (259 builders × 13 ISOs)
...
|--------|------:|
| Artifact kinds | 257 |
| Builder factories | 259 |
| Builder ISO files (13 per kind) | 3,381 |
| Sub-agents (`.claude/agents/`) | 259 |
| Python CLI tools | 152 |
| Pillars | 12 |
...
- **8F pipeline**: every artifact built must show the F1→F8 trace
...
<p align="center">
  ...
  <em>Build your Jarvis. Own your brain.</em>
</p>
